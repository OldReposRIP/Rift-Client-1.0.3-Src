...
public abstract class Expr implements Opcode {
    ...
    public CtBehavior where() {
        ...
        if (ctconstructor != null && ctconstructor.getMethodInfo2() == mi) {
    ...
    public CtClass[] mayThrow() {
        ...
        if (exceptionsattribute != null) {
            String[] astring = exceptionsattribute.getExceptions();
            ...
            if (astring != null) {
                ...
                for (i = 0; i < n; ++i) {
                    try {
                        addClass(list, pool.get(astring[i]));
    ...
    static final void storeStack(CtClass[] names, boolean isStaticCall, int regno, Bytecode bytecode) {
        storeStack0(0, names.length, names, regno + 1, bytecode);
        if (isStaticCall) {
    ...
    private static void storeStack0(int i, int n, CtClass[] names, int regno, Bytecode bytecode) {
        if (i < n) {
            CtClass c = names[i];
            int size;
            ...
            storeStack0(i + 1, n, names, regno + size, bytecode);
            bytecode.addStore(regno, c);
        }
    }

    public abstract void replace(String s) throws CannotCompileException;
