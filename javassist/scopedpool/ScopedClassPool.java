...
public class ScopedClassPool extends ClassPool {
    ...
    protected CtClass getCached(String classname) {
        ...
        if (clazz == null) {
            ...
            if (dcl != null) {
                int registeredCLs = classname.lastIndexOf(36);
                String classResourceName = null;

                if (registeredCLs < 0) {
                    classResourceName = classname.replaceAll("[\\.]", "/") + ".class";
                } else {
                    classResourceName = classname.substring(0, registeredCLs).replaceAll("[\\.]", "/") + classname.substring(registeredCLs) + ".class";
                }
            ...
            if (!isLocal) {
                Map registeredCLs1 = this.repository.getRegisteredCLs();

                synchronized (registeredCLs1) {
                    Iterator it = registeredCLs1.values().iterator();
