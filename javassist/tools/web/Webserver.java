...
public class Webserver {
    ...
    private static final byte[] endofline = new byte[] { (byte) 13, (byte) 10};
    ...
    public void run() {
        ...
        while (true) {
            while (true) {
                try {
                    ServiceThread e = new ServiceThread(this, this.socket.accept());
    ...
    final void process(Socket clnt) throws IOException {
        ...
        try {
            ...
            this.replyError(out, badhttprequest);
        }
        ...
        if (!cmd.startsWith("GET /")) {
            ...
            if (fileType != 2 || !this.letUsersSendClassfile(out, filename, len)) {
                ...
                if (file.canRead()) {
                    ...
                    byte[] barray1 = new byte[4096];

                    while (true) {
                        len = fin1.read(barray1);
                        if (len <= 0) {
                            fin1.close();
                            return;
                        }

                        out.write(barray1, 0, len);
                    }
                } else {
                    if (fileType == 2) {
    ...
    private void checkFilename(String filename, int len) throws BadHttpRequest {
        for (int i = 0; i < len; ++i) {
            ...
            if (!Character.isJavaIdentifierPart(c) && c != 46 && c != 47) {
                throw new BadHttpRequest();
            }
        }
    ...
    private boolean letUsersSendClassfile(OutputStream out, String filename, int length) throws IOException, BadHttpRequest {
        if (this.classPool == null) {
            ...
            try {
                ...
                CtClass e = this.classPool.get(classname);

                classfile = e.toBytecode();
                if (this.debugDir != null) {
                    e.writeFile(this.debugDir);
                }
