...
public class AppletServer extends Webserver {
    ...
    private void processRMI(InputStream ins, OutputStream outs) throws IOException {
        ...
        try {
            ExportedObject out = (ExportedObject) this.exportedObjects.elementAt(objectId);
            Object[] e = this.readParameters(in);

            rvalue = this.convertRvalue(out.methods[methodId].invoke(out.object, e));
        } catch (Exception exception) {
            err = exception;
            this.logging2(exception.toString());
        }

        outs.write(AppletServer.okHeader);
        ObjectOutputStream out1 = new ObjectOutputStream(outs);

        if (err != null) {
            out1.writeBoolean(false);
            out1.writeUTF(err.toString());
        } else {
            try {
                out1.writeBoolean(true);
                out1.writeObject(rvalue);
