...
public class RuntimeSupport {
    ...
    public static String makeDescriptor(Method m) {
        Class[] names = m.getParameterTypes();

        return makeDescriptor(names, m.getReturnType());
    }

    public static String makeDescriptor(Class[] names, Class retType) {
        StringBuffer sbuf = new StringBuffer();

        sbuf.append('(');

        for (int i = 0; i < names.length; ++i) {
            makeDesc(sbuf, names[i]);
        }
    ...
    public static String makeDescriptor(String names, Class retType) {
        StringBuffer sbuf = new StringBuffer(names);
