...
public class JvstTypeChecker extends TypeChecker {
    ...
    public void atMember(Member mem) throws CompileError {
        ...
        if (name.equals(this.codeGen.nameArrayName)) {
            this.exprType = 307;
            this.arrayDim = 1;
            this.className = "java/lang/Object";
    ...
    protected void atFieldAssign(Expr expr, int op, ASTree left, ASTree right) throws CompileError {
        if (left instanceof Member && ((Member) left).get().equals(this.codeGen.nameArrayName)) {
            right.accept(this);
            CtClass[] names = this.codeGen.nameTypeList;
            ...
            for (int i = 0; i < n; ++i) {
                this.compileUnwrapValue(names[i]);
            }
        } else {
    ...
    public boolean isParamListName(ASTList args) {
        if (this.codeGen.nameTypeList != null && args != null && args.tail() == null) {
            ASTree left = args.head();

            return left instanceof Member && ((Member) left).get().equals(this.codeGen.nameListName);
        } else {
            return false;
        }
    }
    ...
    public int getMethodArgsLength(ASTList args) {
        String pname = this.codeGen.nameListName;
        ...
        for (n = 0; args != null; args = args.tail()) {
            ...
            if (a instanceof Member && ((Member) a).get().equals(pname)) {
                if (this.codeGen.nameTypeList != null) {
                    n += this.codeGen.nameTypeList.length;
                }
            } else {
                ++n;
            }
