...
public abstract class CodeGen extends Visitor implements Opcode, TokenId {
    ...
    protected abstract String resolveClassName(String s) throws CompileError;
    ...
    public void atMethodDecl(MethodDecl method) throws CompileError {
        ...
        while (mods != null) {
            Keyword names = (Keyword) mods.head();
        ...
        for (ASTList names1 = method.getParams(); names1 != null; names1 = names1.tail()) {
            this.atDeclarator((Declarator) names1.head());
        }
    ...
    private void atSwitchStmnt(Stmnt st) throws CompileError {
        ...
        for (Object pc = body; pc != null; pc = ((ASTList) pc).tail()) {
            ...
            if (op == 310) {
                ...
                pairs[ipairs++] = ((long) this.computeLabel(endPc.head()) << 32) + ((long) (this.bytecode.currentPc() - opcodePc) & -1L);
            }
        ...
        for (k = 0; k < npairs; ++k) {
            this.bytecode.write32bit(j, (int) (pairs[k] >>> 32));
            this.bytecode.write32bit(j + 4, (int) pairs[k]);
            j += 8;
        }
        ...
        this.bytecode.write32bit(i, defaultPc - opcodePc);
        this.patchGoto(this.breakList, k);
        this.breakList = prevBreakList;
    }
    ...
    private void atVariableAssign(Expr expr, int op, Variable variable, Declarator d, ASTree right, boolean doDup) throws CompileError {
        int i = d.getType();
        int j = d.getArrayDim();
        String s = d.getClassName();
        int k = this.getLocalVar(d);

        if (op != 61) {
            this.atVariable(variable1);
        }

        if (expr == null && right instanceof ArrayInit) {
            this.atArrayVariableAssign((ArrayInit) right, i, j, s);
        } else {
            this.atAssignCore(expr, op, right, i, j, s);
        }
    ...
    protected abstract void atArrayVariableAssign(ArrayInit arrayinit, int i, int j, String s) throws CompileError;
    ...
    protected abstract void atFieldAssign(Expr expr, int i, ASTree astree, ASTree astree1, boolean flag) throws CompileError;
    ...
    protected void atAssignCore(Expr expr, int op, ASTree right, int type, int dim, String cname) throws CompileError {
        if (op == 354 && dim == 0 && type == 307) {
            ...
            if (op != 61) {
                int token = CodeGen.assignOps[op - 351];
    ...
    private boolean invalidDim(int srcType, int srcDim, String srcClass, int destType, int destDim, String destClass, boolean isCast) {
        return srcDim != destDim ? (srcType == 412 ? false : (destDim == 0 && destType == 307 && "java/lang/Object".equals(destClass) ? false : !isCast || srcDim != 0 || srcType != 307 || !"java/lang/Object".equals(srcClass))) : false;
    }
    ...
    static int getCompOperator(ASTree expr) throws CompileError {
        if (expr instanceof Expr) {
            ...
            return token == 33 ? 33 : (bexpr instanceof BinExpr && token != 368 && token != 369 && token != 38 && token != 124 ? 358 : token);
        } else {
            return 32;
        }
    }
    ...
    private static int typePrecedence(int type) {
        return type == 312 ? 0 : (type == 317 ? 1 : (type == 326 ? 2 : (isRefType(type) ? -1 : (type == 344 ? -1 : 3))));
    }
    ...
    protected abstract void atFieldPlusPlus(int i, boolean flag, ASTree astree, Expr expr, boolean flag1) throws CompileError;
