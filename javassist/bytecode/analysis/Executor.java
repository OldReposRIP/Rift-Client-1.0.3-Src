...
public class Executor implements Opcode {
    ...
    public void execute(MethodInfo method, int pos, CodeIterator iter, Frame frame, Subroutine subroutine) throws BadBytecode {
        ...
        case 91:
            ...
            while (j > k) {
                frame.setStack(j, frame.getStack(j - 1));
                --j;
            }

            frame.setStack(k, array);
            break;
        ...
        case 94:
            ...
            while (i > j) {
                frame.setStack(i, frame.getStack(i - 2));
                --i;
            }

            frame.setStack(j, type21);
            frame.setStack(j - 1, type1);
            break;
        ...
        case 132:
            i = iter.byteAt(pos + 1);
            this.verifyAssignable(Type.INTEGER, frame.getLocal(i));
            this.access(i, Type.INTEGER, subroutine);
            break;
    ...
    private void evalInvokeIntfMethod(int opcode, int index, Frame frame) throws BadBytecode {
        ...
        Type[] types = this.nameTypesFromDesc(desc);
        int i = types.length;
    ...
    private Type[] nameTypesFromDesc(String desc) throws BadBytecode {
        ...
        if (classes == null) {
            throw new BadBytecode("Could not obtain nameeters for descriptor [pos = " + this.lastPos + "]: " + desc);
        } else {
