...
public class ControlFlow {
    ...
    public ControlFlow(CtClass ctclazz, MethodInfo minfo) throws BadBytecode {
        ...
        for (i = 0; i < size; ++i) {
            ...
            for (int catchers = 0; catchers < b.exits(); ++catchers) {
                ControlFlow.Block k = b.exit(catchers);

                k.entrances[counters[k.index]++] = b;
            }

            ControlFlow.Catcher[] acontrolflow_catcher = b.catchers();

            for (int i = 0; i < acontrolflow_catcher.length; ++i) {
                ControlFlow.Block catchBlock = acontrolflow_catcher[i].node;
    ...
    public ControlFlow.Node[] dominatorTree() {
        ...
        if (size == 0) {
            ...
            final ControlFlow.Node[] nodes = new ControlFlow.Node[size];
            boolean[] visited = new boolean[size];
            int[] distance = new int[size];

            for (int access = 0; access < size; ++access) {
                nodes[access] = new ControlFlow.Node(this.basicBlocks[access]);
                visited[access] = false;
            }

            ControlFlow.Access controlflow_access = new ControlFlow.Access(nodes) {
            ...
            nodes[0].makeDepth1stTree((ControlFlow.Node) null, visited, 0, distance, controlflow_access);

            do {
                for (int i = 0; i < size; ++i) {
                    visited[i] = false;
                }
            } while (nodes[0].makeDominatorTree(visited, distance, controlflow_access));
    ...
    public ControlFlow.Node[] postDominatorTree() {
        ...
        if (size == 0) {
            ...
            for (int changed = 0; changed < size; ++changed) {
                if (nodes[changed].block.exits() == 0) {
                    counter = nodes[changed].makeDepth1stTree((ControlFlow.Node) null, visited, counter, distance, controlflow_access);
                }
            }
            ...
            do {
                ...
                for (i = 0; i < size; ++i) {
                    if (nodes[i].block.exits() == 0 && nodes[i].makeDominatorTree(visited, distance, controlflow_access)) {
                        flag = true;
                    }
                }
            } while (flag);
    ...
    public static class Node {
        ...
        boolean makeDominatorTree(boolean[] visited, int[] distance, ControlFlow.Access access) {
            ...
            if (visited[index]) {
                ...
                if (exits != null) {
                    for (int entrances = 0; entrances < exits.length; ++entrances) {
                        ControlFlow.Node i = access.node(exits[entrances]);

                        if (i.makeDominatorTree(visited, distance, access)) {
                            changed = true;
                        }
                    }
                }

                BasicBlock[] abasicblock = access.entrances(this);

                if (abasicblock != null) {
                    for (int i = 0; i < abasicblock.length; ++i) {
                        if (this.parent != null) {
                            ControlFlow.Node n = getAncestor(this.parent, access.node(abasicblock[i]), distance);
    ...
    abstract static class Access {
        ...
        abstract BasicBlock[] exits(ControlFlow.Node controlflow_node);

        abstract BasicBlock[] entrances(ControlFlow.Node controlflow_node);
    }
