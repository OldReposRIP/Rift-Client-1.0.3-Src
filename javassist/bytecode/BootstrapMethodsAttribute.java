...
public class BootstrapMethodsAttribute extends AttributeInfo {
    ...
    public BootstrapMethodsAttribute(ConstPool cp, BootstrapMethodsAttribute.BootstrapMethod[] methods) {
        ...
        ByteArray.write16bit(methods.length, abyte, 0);
        int pos = 2;

        for (int i = 0; i < methods.length; ++i) {
            ByteArray.write16bit(methods[i].methodRef, abyte, pos);
            ByteArray.write16bit(methods[i].arguments.length, abyte, pos + 2);
            int[] args = methods[i].arguments;

            pos += 4;

            for (int k = 0; k < args.length; ++k) {
                ByteArray.write16bit(args[k], abyte, pos);
                pos += 2;
            }
        }
