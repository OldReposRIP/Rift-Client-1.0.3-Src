...
final class ByteStream extends OutputStream {
    ...
    private void writeUTF2(String s, int sLen, int offset) {
        ...
        for (pos = offset; pos < sLen; ++pos) {
            char buffer = s.charAt(pos);

            if (buffer > 2047) {
                size += 2;
            } else if (buffer == 0 || buffer > 127) {
                ++size;
            }
        }
        ...
        if (size > '\uffff') {
            ...
            abyte[pos] = (byte) (size >>> 8);
            abyte[pos + 1] = (byte) size;
            pos += 2 + offset;
            ...
            for (int j = offset; j < sLen; ++j) {
                ...
                if (1 <= c && c <= 127) {
                    ...
                    abyte[pos] = (byte) (224 | c >> 12 & 15);
                    abyte[pos + 1] = (byte) (128 | c >> 6 & 63);
                    abyte[pos + 2] = (byte) (128 | c & 63);
                    pos += 3;
                } else {
                    abyte[pos] = (byte) (192 | c >> 6 & 31);
                    abyte[pos + 1] = (byte) (128 | c & 63);
                    pos += 2;
                }
            }
