...
public class AnnotationsAttribute extends AttributeInfo {
    ...
    public AnnotationsAttribute(ConstPool cp, String attrname) {
        this(cp, attrname, new byte[] { (byte) 0, (byte) 0});
    }
    ...
    public void addAnnotation(Annotation annotation) {
        ...
        Annotation[] aannotation = new Annotation[annotations.length + 1];

        System.arraycopy(annotations, 0, aannotation, 0, annotations.length);
        aannotation[annotations.length] = annotation;
        this.setAnnotations(aannotation);
    }
    ...
    static class Parser extends AnnotationsAttribute.Walker {
        ...
        void nameeters(int numParam, int pos) throws Exception {
            Annotation[][] names = new Annotation[numParam][];

            for (int i = 0; i < numParam; ++i) {
                pos = this.annotationArray(pos);
                names[i] = this.allAnno;
            }

            this.allParams = names;
        }
    ...
    static class Walker {
        ...
        final void nameeters() throws Exception {
            int numParam = this.info[0] & 255;

            this.nameeters(numParam, 1);
        }
